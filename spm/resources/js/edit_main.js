...
// 모바일 <-> PC 상태가 바뀐 경우
    ...
    numberButtons.forEach((button) => {
        button.addEventListener('click', (event) => {
            ...
            // 입력 필드 업데이트 (최신값 설정)
            inputField.value = inputValues; // 입력된 값은 *로 표시
        });
    });
    ...
    /*-----------------------------------------------------------------------------------------------------*/
    // 전역 에러 처리기
    function showErrorModal(modalName, errorMessage) {
        ...
        if (modal) {
            ...
            newModal.id = modalName;
            newModal.className = 'modal pd-sm';
            newModal.setAttribute('data-modal-type', 'confirm-ver');
            newModal.setAttribute('role', 'dialog');
            newModal.setAttribute('aria-labelledby', `${modalName}-title`);
            newModal.setAttribute('aria-describedby', `${modalName}-desc`);
            newModal.setAttribute('aria-hidden', 'true');

            newModal.innerHTML = `
      <div class="modal-content sm pd-auto" tabindex="-1">
        <div class="confirm-box">
          <img src="/spm2/images/ico/ico_error.svg" alt="오류 아이콘" class="ico-error">
          <p class="c-em error-text">${errorMessage}</p>
        </div>
        <div class="flex-wrap center">
          <button type="button" class="btn size-nm" onclick="hideModal('${modalName}')">확인</button>
        </div>
      </div>
    `;
    ...
    const GlobalErrorHandler = {
        handle(error, context = {}) {
            // 모달 띄우기
            showErrorModal("globalErrorModal", error && error.message ? error.message : String(error));
            // 필요하면 콘솔 로그도
            console.error('[GlobalError]', error, context);
        }
    };

    // 2) window.onerror
    window.onerror = function(message, source, lineno, colno, error) {
        GlobalErrorHandler.handle(error || {message}, {type: 'onerror', source, lineno, colno});
        return false;
    };

    // 3) unhandledrejection
    window.addEventListener('unhandledrejection', event => {
        GlobalErrorHandler.handle(event.reason, {type: 'unhandledrejection'});
    });

    // 4) 리소스 로드 에러
    window.addEventListener('error', event => {
        if (event.target && (event.target.src || event.target.href)) {
            GlobalErrorHandler.handle(new Error('Resource load error'), {
                type: 'resourceError',
    ...
    if (window.fetch) {
        ...
        window.fetch = function(...args) {
            return _fetch.apply(this, args).catch(err => {
                GlobalErrorHandler.handle(err, {type: 'fetch', args});
    ...
    (function() {
        ...
        EventTarget.prototype.addEventListener = function(type, listener, options) {
            orig.call(this, type, event => {
                try {
                    listener.call(this, event);
                } catch (err) {
                    GlobalErrorHandler.handle(err, {type: 'DOMEvent', eventType: type, element: this});
                }
            }, options);
        };
    })();
